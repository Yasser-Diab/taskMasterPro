-- Height is private account profile data used to estimate distance when a
-- health source supplies steps but no distance record.
...
comment on column public.profiles.height_cm is
  'Private height in centimetres for user-approved wellbeing calculations.';

create or replace function taskmaster_internal.apply_profile_merge_command(
  p_command_id uuid,
  p_device_id uuid,
...
begin
  ...
  set
    ...
    height_cm = case
      when p_payload ? 'height_cm'
        then nullif(p_payload ->> 'height_cm', '')::numeric
      else height_cm
    end,
...
revoke all on function taskmaster_internal.apply_profile_merge_command(
  uuid, uuid, bigint, bigint, jsonb
) from public, anon;
grant execute on function taskmaster_internal.apply_profile_merge_command(
  uuid, uuid, bigint, bigint, jsonb

...
namespace
{
  ...
  DWORD FlutterSecureStorageWindowsPlugin::GetApplicationSupportPath(std::wstring &path)
  {
      ...
      if (versionInfoSize != 0) {
          ...
          // DayVector keeps the established encrypted-vault location across
          // the public product rename. These values are an internal storage
          // identity only; visible executable metadata is branded DayVector.
          companyName = L"pro.taskmaster";
          productName = L"TaskMaster Pro";
